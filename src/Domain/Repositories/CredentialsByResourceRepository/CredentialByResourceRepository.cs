...
public class CredentialByResourceRepository :
    CassandraRepositoryBase<CredentialByResourceEntity>,
    ICredentialByResourceRepository
{
    public CredentialByResourceRepository(ICassandraSessionFactory sessionFactory,
        ILogger<CassandraRepositoryBase<CredentialByResourceEntity>> logger)
        : base(sessionFactory, logger)
    {
    }
    ...
    {
        var existedCredentialByResourceEntities =
            await TryGetCredentialByResourceAsync(credentialByResourceEntity);
        ...
        CredentialByResourceEntity credentialByResourceEntity)
    {
        var existedCredentialByResourceEntities = (await ExecuteQueryAsync(Table
                .Where(e => e.UserLogin == credentialByResourceEntity.UserLogin
                            ...
                            && e.ResourceLogin == credentialByResourceEntity.ResourceLogin)))
            .ToArray();
    ...
    public CqlCommand CreateCredentialByResourceQuery(
        CredentialByResourceEntity credentialByResourceEntity)
    {
